...
document.addEventListener('DOMContentLoaded', () => {
    ...
    linkForgot.addEventListener('click', (e) => {
        ...
        // Reset steps
        fpSteps.forEach((s, idx) => {
            if (idx === 0) {
                s.classList.add('active-step');
                s.classList.remove('icon-hidden');
            } else {
                s.classList.remove('active-step');
                s.classList.add('icon-hidden');
            }
        });
    });
    ...
    if (userProfileMenu && logoutDropdown) {
        userProfileMenu.addEventListener('click', (e) => {
            logoutDropdown.classList.toggle('icon-hidden');
            e.stopPropagation();
        });
        document.addEventListener('click', (e) => {
            if (!userProfileMenu.contains(e.target)) {
                logoutDropdown.classList.add('icon-hidden');
            }
        });
    }
    ...
    async function initializeDatabaseAndData() {
        const mapCustomers = async () => {
            const rows = await allQuery('SELECT id as dbId, code, name, phone, address, orders, total as totalVal, note FROM Customers ORDER BY id DESC');
            return rows.map(r => ({ ...r, total: new Intl.NumberFormat('vi-VN').format(r.totalVal) + ' đ' }));
        }
        
        const mapSuppliers = async () => {
             return await allQuery('SELECT id as dbId, code, name, contact, phone, address FROM Suppliers ORDER BY id DESC');
        }

        const mapProducts = async () => {
            return await allQuery('SELECT id as dbId, code as id, name, unit, priceStr as price, priceRaw, importPrice, markupPercent, stock, status, expiryDate FROM Products ORDER BY id DESC');
        }

        const mapSales = async () => {
            const rows = await allQuery("SELECT id as dbId, code as id, date, entityName as customer, phone, address, itemsCount as items, totalStr as total, totalRaw, itemsStr FROM Transactions WHERE type='SALE' ORDER BY id DESC");
            for (let r of rows) {
                r.itemsDetail = await allQuery('SELECT productCode as productId, productName as name, unit, qty, price, importPrice, total FROM TransactionDetails WHERE transactionCode=?', [r.id]);
            }
            return rows;
        }

        const mapImports = async () => {
            const rows = await allQuery("SELECT id as dbId, code as id, date, entityName as supplier, phone, address, itemsCount as items, totalStr as total, totalRaw FROM Transactions WHERE type='IMPORT' ORDER BY id DESC");
            for (let r of rows) {
                r.itemsDetail = await allQuery('SELECT productCode as productId, productName as name, qty, price, total FROM TransactionDetails WHERE transactionCode=?', [r.id]);
            }
            return rows;
        }

        customerData = await mapCustomers();

        if (customerData.length === 0) {
            console.log("No data found, starting seed...");
            const cData = generateCustomerData();
            const sData = generateSupplierData();
            const pData = generateProductData();
            const { sales: slData, imports: iData } = generateTransactionalData(pData, cData, sData);
            ...
            for (const s of sData) {
                await runQuery('INSERT INTO Suppliers (code, name, contact, phone, address) VALUES (?,?,?,?,?)', [s.code, s.name, s.contact, s.phone, s.address]);
            }
            for (const p of pData) {
                await runQuery('INSERT INTO Products (code, name, unit, priceStr, priceRaw, importPrice, stock, status, expiryDate) VALUES (?,?,?,?,?,?,?,?,?)', [p.id, p.name, p.unit, p.price, p.priceRaw, p.importPrice, p.stock, p.status, p.expiryDate]);
            }
            for (const t of slData) {
                await runQuery('INSERT INTO Transactions (code, type, date, entityName, phone, address, itemsCount, totalStr, totalRaw, itemsStr) VALUES (?,?,?,?,?,?,?,?,?,?)', [t.id, 'SALE', t.date, t.customer, t.phone, t.address, t.items, t.total, t.totalRaw, t.itemsStr]);
                for (const d of t.itemsDetail) {
                    await runQuery('INSERT INTO TransactionDetails (transactionCode, productCode, productName, unit, qty, price, importPrice, total) VALUES (?,?,?,?,?,?,?,?)', [t.id, d.productId, d.name, d.unit, d.qty, d.price, d.importPrice, d.total]);
                }
            }
            for (const t of iData) {
                await runQuery('INSERT INTO Transactions (code, type, date, entityName, phone, address, itemsCount, totalStr, totalRaw) VALUES (?,?,?,?,?,?,?,?,?)', [t.id, 'IMPORT', t.date, t.supplier, t.phone, t.address, t.items, t.total, t.totalRaw]);
                for (const d of t.itemsDetail) {
                     await runQuery('INSERT INTO TransactionDetails (transactionCode, productCode, productName, unit, qty, price, total) VALUES (?,?,?,?,?,?,?)', [t.id, d.productId, d.name, null, d.qty, d.price, d.total]);
                }
            }
        ...
        supplierData = await mapSuppliers();
        productData = await mapProducts();
        salesData = await mapSales();
        importData = await mapImports();
                ...
                <div class="ticket-card" data-type="sales" data-id="${item.id}">
                    <div class="card-header-row">
                        ...
                        <div class="card-actions">
                            <button class="icon-btn btn-edit" data-type="sales" data-id="${item.id}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                            <button class="icon-btn delete" data-type="sales" data-id="${item.id}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
                        </div>
                    </div>
            ...
            container.innerHTML = currentData.map(item => `
                <div class="ticket-card" data-type="imports" data-id="${item.id}">
                     <div class="card-header-row">
                        <span class="ticket-id">${item.id}</span>
                        <div class="card-actions">
                            <button class="icon-btn btn-edit" data-type="imports" data-id="${item.id}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                            <button class="icon-btn delete" data-type="imports" data-id="${item.id}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
                        </div>
                    </div>
                    <div class="ticket-date">Ngày: ${item.date}</div>
                    <div class="ticket-row">
                        ...
                        <tr>
                            ...
                            <td style="text-align: center">
                                 <div class="card-actions" style="justify-content: center">
                                    <button class="icon-btn btn-edit" data-id="${item.code}" data-type="customers"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                ...
                <table class="entity-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center">
                                 <div class="card-actions" style="justify-content: center">
                                    <button class="icon-btn btn-edit" data-id="${item.code}" data-type="suppliers"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                ...
                <table class="entity-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center"><span class="status-badge ${item.stock > 10 ? 'status-green' : (item.stock > 0 ? 'status-orange' : 'status-red')}">${item.stock > 10 ? 'Còn hàng' : (item.stock > 0 ? 'Sắp hết hàng' : 'Hết hàng')}</span></td>
                            <td style="text-align: center">
                                 <div class="card-actions" style="justify-content: center">
        ...
        renderFields(fields) {
            ...
            fields.forEach(field => {
                ...
                if (field.type === 'custom_unit') {
                    ...
                    input.id = 'modal-input-' + field.key;
                    if (field.readOnly) { input.readOnly = true; input.style.background = '#f5f5f5'; }
                    input.className = 'input-field-styled modal-input-rounded';
                    input.placeholder = field.placeholder || '';
        ...
        btnOpenAddProduct.addEventListener('click', () => {
            modalForm.open({
                ...
                fields: [
                    ...
                    { key: 'importPrice', label: 'Giá nhập', type: 'number', required: false, value: '0', marginTop: '15px' },
                    { key: 'markupPercent', label: '% Lợi nhuận', type: 'number', required: false, value: '0', marginTop: '15px' },
                    { key: 'price', label: 'Giá bán', type: 'text', required: false, value: '0', marginTop: '15px', readOnly: true },
                    { key: 'expiryDate', label: 'Ngày hết hạn', type: 'date', required: false, value: '', marginTop: '15px' },
                ],
                onSubmit: (data) => {
                    ...
                    const pImport = parseFloat(data.importPrice) || 0;
                    const pMarkup = parseFloat(data.markupPercent) || 0;
                    let pValRaw = pImport * (1 + pMarkup / 100);
                    pValRaw = roundPriceCustom(pValRaw);
                    const formattedPrice = formatCurrency(pValRaw);
                    ...
                    runQuery('INSERT INTO Products (code, name, unit, priceStr, priceRaw, importPrice, markupPercent, stock, status, expiryDate) VALUES (?,?,?,?,?,?,?,?,?,?)',
                        [code, data.name, data.unit, formattedPrice, pValRaw, pImport, pMarkup, 0, 'Hết hàng', expStr]
                    ).then(() => {
            ...
            // Reactivity setup
            setTimeout(() => {
                const iImport = document.getElementById('modal-input-importPrice');
                const iMarkup = document.getElementById('modal-input-markupPercent');
                const iPrice = document.getElementById('modal-input-price');

                const updatePrice = () => {
                    if (iImport && iMarkup && iPrice) {
                        const imp = parseFloat(iImport.value) || 0;
                        const mkp = parseFloat(iMarkup.value) || 0;
                        let out = imp * (1 + mkp / 100);
                        out = roundPriceCustom(out);
                        iPrice.value = out;
                        modalForm.state['price'] = out;
                    }
                };
                if (iImport) iImport.addEventListener('input', updatePrice);
                if (iMarkup) iMarkup.addEventListener('input', updatePrice);
            }, 100);
        });
    }
    ...
    window.editProduct = function (id) {
        ...
        // Convert exp from DD/MM/YYYY to YYYY-MM-DD
        let editExpVal = '';
        if (product.expiryDate) {
            const parts = product.expiryDate.split('/');
            if(parts.length === 3) editExpVal = `${parts[2]}-${parts[1]}-${parts[0]}`;
        }
        ...
        modalForm.open({
            ...
            fields: [
                ...
                { key: 'importPrice', label: 'Giá nhập', type: 'number', required: false, value: product.importPrice || 0, marginTop: '15px' },
                { key: 'markupPercent', label: '% Lợi nhuận', type: 'number', required: false, value: product.markupPercent || 0, marginTop: '15px' },
                { key: 'price', label: 'Giá bán', type: 'text', required: false, value: priceNum, marginTop: '15px', readOnly: true },
                { key: 'stock', label: 'Tồn kho', type: 'number', required: false, value: product.stock, marginTop: '15px' },
                { key: 'expiryDate', label: 'Ngày hết hạn', type: 'date', required: false, value: editExpVal, marginTop: '15px' }
            ],
            onSubmit: (data) => {
                ...
                if (index !== -1) {
                    ...
                    runQuery('UPDATE Products SET name=?, unit=?, priceStr=?, priceRaw=?, importPrice=?, markupPercent=?, stock=?, status=?, expiryDate=? WHERE code=?', 
                        [data.name, data.unit, formattedPrice, pValRaw, pImport, pMarkup, productData[index].stock, productData[index].status, expStr, id]
                    ).then(() => {
        ...
        btnOpenAddCustomer.addEventListener('click', () => {
            modalForm.open({
                ...
                onSubmit: (data) => {
                    ...
                    runQuery('INSERT INTO Customers (code, name, phone, address, note, orders, total) VALUES (?,?,?,?,?,?,?)',
                        [code, data.name, data.phone, data.address, data.note || '', 0, 0]
                    ).then(() => {
        ...
        btnOpenAddSupplier.addEventListener('click', () => {
            modalForm.open({
                ...
                onSubmit: (data) => {
                    ...
                    runQuery('INSERT INTO Suppliers (code, name, contact, phone, address) VALUES (?,?,?,?,?)',
                        [code, data.name, data.contact, data.phone, data.address]
                    ).then(() => {
        ...
        modalForm.open({
            ...
            onSubmit: (data) => {
                ...
                if (index !== -1) {
                    ...
                    runQuery('UPDATE Customers SET name=?, phone=?, address=?, note=? WHERE code=?',
        ...
        modalForm.open({
            ...
            onSubmit: (data) => {
                ...
                if (index !== -1) {
                    ...
                    runQuery('UPDATE Suppliers SET name=?, contact=?, phone=?, address=? WHERE code=?',
                        [data.name, data.contact, data.phone, data.address, id]
                    ).then(() => {
    ...
    class TransactionFormHandler {
        constructor() {
            ...
            // Fields
            this.nameLabel = document.getElementById('tm-edit-name-label');
        ...
        bindEvents() {
            ...
            if (this.nameInput) this.nameInput.addEventListener('input', () => this.validate());
        }
        ...
        open(type, id) {
            ...
            if (this.nameLabel) {
                this.nameLabel.textContent = type === 'sales' ? 'Tên khách hàng' : 'Nhà cung cấp';
            }

            if (this.nameInput) {
                this.nameInput.innerHTML = '<option value="">-- Chọn --</option>';
                const sourceData = type === 'sales' ? filteredCustomerData : filteredSupplierData;
                sourceData.forEach(item => {
                    ...
                    this.nameInput.appendChild(opt);
                });

                this.nameInput.onchange = (e) => {
                    const opt = e.target.options[e.target.selectedIndex];
                    if (opt && opt.value) {
                        if (this.phoneInput) this.phoneInput.value = opt.dataset.phone;
                        if (this.addressInput) this.addressInput.value = opt.dataset.address;
                    } else {
            ...
            if (id === null) {
                ...
                // Populate Header
                if (this.nameInput) this.nameInput.value = type === 'sales' ? data.customer : data.supplier;
                ...
                // Populate Items
                this.items = data.itemsDetail && Array.isArray(data.itemsDetail) 
                    ? JSON.parse(JSON.stringify(data.itemsDetail)) 
                    : this.parseItemsString(data.itemsStr);
            }
        ...
        getMaxAllowedStock(index, productName) {
            if (this.currentType !== 'sales') return Infinity;
            if (!productName) return Infinity;
            ...
            if (this.currentId !== null) {
                const pIndex = salesData.findIndex(i => i.id == this.currentId);
                if (pIndex !== -1 && salesData[pIndex].itemsDetail) {
                    // It's possible the old transaction had multiple rows of the same product, so we sum them
                    const oldTotal = salesData[pIndex].itemsDetail
                        .filter(old => old.name === productName)
                        .reduce((sum, old) => sum + old.qty, 0);
                    maxStock += oldTotal;
                }
            }
            ...
            this.items.forEach((it, i) => {
                if (i !== index && it.name === productName) {
                    otherRowsSum += it.qty;
                }
            });

            maxStock -= otherRowsSum;
            return maxStock < 0 ? 0 : maxStock;
        }
        ...
        validate() {
            ...
            rows.forEach((tr, index) => {
                ...
                const realInputName = tr.querySelector('td:nth-child(1) select') || tr.querySelector('td:nth-child(1) input'); // Name
                ...
                if (item.qty <= 0) {
                    ...
                    isValid = false;
                } else if (this.currentType === 'sales' && item.name) {
                    const maxStock = this.getMaxAllowedStock(index, item.name);
                    if (item.qty > maxStock) {
                        this.showError(realInputQty, `Vượt tồn (${maxStock})`);
                        isValid = false;
                    } else {
        ...
        renderRows() {
            ...
            let dataList = document.getElementById('tm-product-datalist');
            if (!dataList) {
                dataList = document.createElement('datalist');
                dataList.id = 'tm-product-datalist';
                document.body.appendChild(dataList);
            }
            dataList.innerHTML = productData.map(p => `<option value="${p.name}">`).join('');
            ...
            this.items.forEach((item, index) => {
                ...
                inputName.style.appearance = 'auto';
                inputName.style.backgroundColor = '#fff';
                
                inputName.innerHTML = '<option value="">-- Chọn hàng hoá --</option>' + 
                    productData.map(p => {
                        const isSelected = (p.name === item.name) ? 'selected' : '';
                        return `<option value="${p.name}" ${isSelected}>${p.name}</option>`;
                    }).join('');
                inputName.onchange = (e) => {
                    const val = e.target.value;
                    this.items[index].name = val;
                    const product = productData.find(p => p.name === val);
                    if (product) {
                        this.items[index].unit = product.unit;
                        this.items[index].price = this.currentType === 'sales' ? product.priceRaw : (product.importPrice || 0);
                ...
                inputQty.type = 'text'; // Use text for strict regex control
                ...
                const updateQtyFromButton = (newVal) => {
                    ...
                    if (this.currentType === 'sales' && this.items[index].name) {
                        const maxStock = this.getMaxAllowedStock(index, this.items[index].name);
                        if (newVal > maxStock) newVal = maxStock;
                    }
                    
                    this.items[index].qty = newVal;
                    inputQty.value = newVal; // Update input visual
                    this.updateRowDOM(index);
                    this.validate();
                };
                ...
                inputUnit.type = 'text';
                inputUnit.className = 'input-field-styled';
                ...
                inputPrice.className = 'input-field-styled';
        ...
        save() {
            ...
            if (this.currentId === null) {
                ...
                const newObj = {
                    ...
                    itemsStr: itemsStr,
                    itemsDetail: JSON.parse(JSON.stringify(this.items)),
                    total: totalStr,
                    totalRaw: totalVal,
                ...
                const typeDb = this.currentType === 'sales' ? 'SALE' : 'IMPORT';
                runQuery('INSERT INTO Transactions (code, type, date, entityName, phone, address, itemsCount, totalStr, totalRaw, itemsStr) VALUES (?,?,?,?,?,?,?,?,?,?)', 
                    [newId, typeDb, newObj.date, newName, newPhone, newAddress, this.items.length, totalStr, totalVal, itemsStr]
                ).then(() => {
                    this.items.forEach(d => {
                        runQuery('INSERT INTO TransactionDetails (transactionCode, productName, unit, qty, price, total) VALUES (?,?,?,?,?,?)', 
                        [newId, d.name, d.unit, d.qty, d.price, d.total]).catch(console.error);
                        
                        // SYNC STOCK FOR NEW AND ADJUST IMPORT PRICING
                        const delta = this.currentType === 'sales' ? -d.qty : d.qty;
                        const product = productData.find(p => p.name === d.name);
                        if (product) {
                            product.stock = parseInt(product.stock || 0, 10) + parseInt(delta || 0, 10);
                            
                            if (this.currentType === 'imports') {
                                // Auto Update Pricing Logic based on new import cost
                                product.importPrice = d.price; // d.price is the unit price from the import ticket
                                let pValRaw = product.importPrice * (1 + (product.markupPercent || 0) / 100);
                                pValRaw = roundPriceCustom(pValRaw);
                                product.priceRaw = pValRaw;
                                product.priceStr = formatCurrency(pValRaw);
                                product.price = product.priceStr; // update ui field
                                
                                runQuery('UPDATE Products SET stock = ?, importPrice = ?, priceRaw = ?, priceStr = ? WHERE code = ?', 
                                    [product.stock, product.importPrice, product.priceRaw, product.priceStr, product.id]).catch(console.error);
                            } else {
                                // Standard Sales Deduction
                                runQuery('UPDATE Products SET stock = ? WHERE code = ?', [product.stock, product.id]).catch(console.error);
                            }
                        }
                    });
                }).catch(console.error);
                ...
                let pIndex = -1;
                if (this.currentType === 'sales') {
                    pIndex = salesData.findIndex(i => i.id == this.currentId);
                    if (pIndex !== -1) {
                        oldItems = salesData[pIndex].itemsDetail || [];
                        salesData[pIndex] = {
                            ...salesData[pIndex],
                            customer: newName,
                            phone: newPhone,
                    ...
                    pIndex = importData.findIndex(i => i.id == this.currentId);
                    if (pIndex !== -1) {
                        oldItems = importData[pIndex].itemsDetail || [];
                        importData[pIndex] = {
                            ...importData[pIndex],
                            supplier: newName,
                            phone: newPhone,
                ...
                runQuery('UPDATE Transactions SET entityName=?, phone=?, address=?, itemsCount=?, totalStr=?, totalRaw=?, itemsStr=? WHERE code=?', 
                    [newName, newPhone, newAddress, this.items.length, totalStr, totalVal, itemsStr, this.currentId]
                ).then(() => {
                    runQuery('DELETE FROM TransactionDetails WHERE transactionCode=?', [this.currentId]).then(() => {
                        this.items.forEach(d => {
                            runQuery('INSERT INTO TransactionDetails (transactionCode, productName, unit, qty, price, total) VALUES (?,?,?,?,?,?)', 
                            [this.currentId, d.name, d.unit, d.qty, d.price, d.total]).catch(console.error);
                            ...
                            if (product) {
                                product.stock = parseInt(product.stock || 0, 10) + parseInt(delta || 0, 10);
                                if (this.currentType === 'imports') {
                                    product.importPrice = d.price;
                                    let pValRaw = product.importPrice * (1 + (product.markupPercent || 0) / 100);
                                    pValRaw = roundPriceCustom(pValRaw);
                                    product.priceRaw = pValRaw;
                                    product.priceStr = formatCurrency(pValRaw);
                                    product.price = product.priceStr;
                ...
                if (dataArray && renderFunc) {
                    ...
                    if (index !== -1) {
                        ...
                        if (type === 'sales' || type === 'imports') {
                            const deletedTx = dataArray[index];
                            if (deletedTx.itemsDetail) {
                                deletedTx.itemsDetail.forEach(d => {
                                    const delta = type === 'sales' ? d.qty : -d.qty;
                                    const product = productData.find(p => p.name === d.name);
                                    if (product) {
                                        product.stock = parseInt(product.stock || 0, 10) + parseInt(delta || 0, 10);
                                        runQuery('UPDATE Products SET stock = ? WHERE code = ?', [product.stock, product.id]).catch(console.error);
                        ...
                        // Delete DB Side
                        let tableName = '';
                        if (type === 'sales' || type === 'imports') tableName = 'Transactions';
                        else if (type === 'customers') tableName = 'Customers';
                        else if (type === 'suppliers') tableName = 'Suppliers';
                        else if (type === 'products') tableName = 'Products';
                        
                        runQuery(`DELETE FROM ${tableName} WHERE code=?`, [id]).catch(console.error);
    ...
    function renderProductExpiryManagement() {
        ...
        const getDaysLeft = (expStr) => {
            if (!expStr) return Infinity; // No expiry date means very safe
        ...
        container.innerHTML = `
            ...
            if (days === Infinity) {
                statusStr = 'An toàn';
                badgeStyle = 'background:#E8F5E9; color:#2E7D32;'; // Green
            } else if (days < 0) {
                statusStr = 'Hết hạn';
    ...
    const showTransactionDetail = (type, id) => {
        ...
        if (container) {
            ...
            if (item.itemsDetail && item.itemsDetail.length > 0) {
                listContainer.innerHTML = item.itemsDetail.map(d => `
                    <div class="tm-item">
                        <div class="tm-item-name">${d.name}</div>
                        <div class="tm-item-row">
                             <div class="tm-col tm-info-card">
                                 <label>Số lượng</label>
                                 <span>${d.qty}</span>
                            ...
                            <div class="tm-col tm-info-card">
                                <label>Thành tiền</label>
                                <span>${formatCurrency(d.qty * d.price)}</span>
                            </div>
                        </div>
                    </div>
                `).join('');
    ...
    function renderRevenueReport(data, timeScale, container) {
        ...
        data.forEach(item => {
            ...
            if (item.itemsDetail && Array.isArray(item.itemsDetail) && item.itemsDetail.length > 0) {
                item.itemsDetail.forEach(d => {
                    const importP = typeof d.importPrice === 'number' && d.importPrice > 0 ? d.importPrice : (d.price * 0.7);
                    const profit = (d.price - importP) * d.qty;
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center;">
                                        <span class="status-badge ${p.stock == 0 ? 'status-red' : (p.stock < 10 ? 'status-orange' : 'status-green')}">
                                            ${p.stock > 10 ? 'Còn hàng' : (p.stock > 0 ? 'Sắp hết hàng' : 'Hết hàng')}
                                        </span>
                                    </td>
                                </tr>
                            `).join('')}
